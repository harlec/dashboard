      ...
      {compact ? (
        <div style={{ padding: '10px 16px', borderBottom: '1px solid rgba(239,75,84,.14)', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <span style={{ fontFamily: 'var(--app-font)', fontSize: 11, letterSpacing: '.14em', color: '#7d8a9c', fontWeight: 700, textTransform: 'uppercase' as const }}>
              Equipos Caídos
            </span>
          </div>
          <span style={{ fontFamily: 'var(--app-font)', fontSize: 12, fontWeight: 700, padding: '2px 10px', borderRadius: 20, background: caidos.length > 0 ? 'rgba(239,75,84,.15)' : 'rgba(63,185,120,.12)', color: caidos.length > 0 ? '#ef4b54' : '#3fb978', border: `1px solid ${caidos.length > 0 ? 'rgba(239,75,84,.35)' : 'rgba(63,185,120,.35)'}` }}>
      ...
      {caidos.length === 0 ? (
        ...
        <div style={{ flex: 1, overflowY: 'auto', minHeight: 0 }}>
          {caidos.map(({ eq, estacion }, idx) => (
            ...
            >
              <span style={{ fontFamily: 'var(--app-font)', fontSize: 11, color: 'rgba(255,255,255,.22)', width: 18, flexShrink: 0 }}>{idx + 1}</span>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ fontFamily: 'var(--app-font)', fontSize: 11, color: '#5f7186', marginTop: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{estacion} · {eq.tipoNombre}</div>
              </div>
              <span style={{ fontFamily: 'var(--app-font)', fontSize: 13, fontWeight: 700, flexShrink: 0, color: (eq.incMin ?? 0) > 60 ? '#ef4b54' : '#e0991f' }}>
                {dur(eq.incMin)}
              </span>
            </div>
          ))}
          ...
          <tbody>
            {caidos.map(({ eq, estacion, via }, i) => (
              ...
              >
                ...
                <td className="px-4 py-3 text-[0.78rem] text-[#d4cec9]">{eq.ip}</td>
                <td className="px-4 py-3">

  ...
  return (
    <div style={{ background: 'linear-gradient(180deg,#0c141d,#080e15)', border: '1px solid rgba(45,212,167,.12)', borderRadius: 12, overflow: 'hidden', display: 'flex', flexDirection: 'column', height: '100%', minHeight: 0 }}>
      <div style={{ padding: '10px 18px', borderBottom: '1px solid rgba(45,212,167,.1)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
        <span style={{ fontFamily: 'var(--app-font)', fontSize: 11, letterSpacing: '.14em', color: '#5a8a7a', fontWeight: 700, textTransform: 'uppercase' }}>
          {title}
        </span>
        <span style={{ fontFamily: 'var(--app-font)', fontSize: 10, color: '#2a4040' }}>{sub}</span>
      </div>
      {children}
    </div>
  )
          ...
          return (
            <g key={r.name}>
              ...
              <text x={ML + barW + 6} y={y + RH / 2 + 4}
                fill={r.color} fontSize="10" fontWeight="600" fontFamily="var(--app-font)">
                {r.lat != null ? `${r.lat} ms` : '—'}
              </text>
            </g>
          )
          ...
          return (
            <g key={est.id}>
              ...
              <text x={W - MR + 4} y={y + RH / 2 + 4}
                fill={col} fontSize="10" fontWeight="600" fontFamily="var(--app-font)">
                {est.up}/{total}
              </text>
            </g>
          )
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, padding: '10px 14px 14px' }}>
        {[
          ...
          <div key={label} style={{ background: 'rgba(255,255,255,.03)', border: '1px solid rgba(255,255,255,.06)', borderRadius: 8, padding: '8px 10px', textAlign: 'center' }}>
            <div style={{ fontFamily: 'var(--app-font)', fontSize: 16, fontWeight: 700, color, letterSpacing: '.01em' }}>
              {val}
            </div>
            <div style={{ fontFamily: 'var(--app-font)', fontSize: 10, letterSpacing: '.12em', color: '#4a6070', marginTop: 4 }}>
              {label}
            </div>
          </div>
        ))}

...
// (allá "Gamma" se llama "GAMMA" — ver EstacionCase en DiscrepanciasService.cs)
const ORDEN_ESTACIONES = ['Alfa', 'Beta', 'Gamma', 'Epsilon', 'Delta']
...
export function NocMuro() {
  ...
  //
  // DEMO_MODE: en producción los incidentes reales son poco frecuentes, así que
  // 5 min de por medio es razonable. Acá el ping simulado genera caídas/recuperaciones
  // cada pocos segundos (ver DemoMode en PingWorkerService.cs) y con 5 min la cola
  // "Requieren atención" casi nunca alcanza a mostrar nada — se acorta a 15s solo
  // en esta instancia demo para que la demo se vea reactiva en vivo.
  useEffect(() => {
    const cargar = () => {

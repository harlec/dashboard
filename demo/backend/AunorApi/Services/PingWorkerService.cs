    ...
    ILogger<PingWorkerService> log) : BackgroundService
{
    ...
    // ── Modo demo ───────────────────────────────────────────────────────
    // Instancia de demostración sin red/datos reales (ver docker-compose.demo.yml,
    // DEMO_MODE=true): las IPs de los equipos son de ejemplo (192.168.x.x) y no
    // responden a nada, así que en vez de pingarlas de verdad se simula un
    // resultado realista por IP (mayormente UP, caídas y recuperaciones ocasionales).
    ...
    private static readonly ConcurrentDictionary<string, bool> DemoEstadoUp = new();
    private static readonly Random DemoRandom = new();

    private static (string estado, double? latencia, string? detalle) SimulateCheck(string ip)
    {
        bool up = DemoEstadoUp.GetOrAdd(ip, true);
        int roll;
        lock (DemoRandom) { roll = up ? DemoRandom.Next(1000) : DemoRandom.Next(100); }

        if (up)
        {
            if (roll < 15) { DemoEstadoUp[ip] = false; return ("DOWN", null, "TimedOut"); }   // ~1.5% de caer por ciclo
            double lat;
            lock (DemoRandom) { lat = 5 + DemoRandom.NextDouble() * 35; }
            return ("UP", lat, null);
        }

        if (roll < 30) { DemoEstadoUp[ip] = true; double lat; lock (DemoRandom) { lat = 5 + DemoRandom.NextDouble() * 35; } return ("UP", lat, null); }
        return ("DOWN", null, "TimedOut");   // ~30% de recuperarse por reintento
    }
    ...
    {
        if (DemoMode)
            return Task.FromResult(SimulateCheck(ip));

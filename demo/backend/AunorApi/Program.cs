...
// ── Background services ───────────────────────────────────────
// Instancia demo (DEMO_MODE=true, ver docker-compose.demo.yml): PingWorkerService
// se queda (simula resultados en vez de pingar IPs reales, ver CheckHost), pero los
// workers que hacen llamadas de red externas reales (correo IMAP de cámaras, ping ICMP
// de enlaces, Telegram, reporte semanal por email) no tienen nada real que monitorear
// ni a quién avisar, así que no se registran.
bool demoMode = string.Equals(Environment.GetEnvironmentVariable("DEMO_MODE"), "true", StringComparison.OrdinalIgnoreCase);
builder.Services.AddHostedService<PingWorkerService>();
builder.Services.AddSingleton<ReporteSemanalService>();   // inyectado por ReporteController — se registra siempre
if (!demoMode)
{

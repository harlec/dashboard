...
-- ── servicios ────────────────────────────────────────────────
-- Servicios monitoreados que no cuelgan de una vía/estación (servidores en
-- sala — Lima/Chimbote —, páginas web). Cada servicio agrupa uno o más
-- "criterios" de chequeo (servicio_checks): ping a una IP, puerto TCP, o
...
CREATE TABLE servicio_checks (
    ...
    nombre             NVARCHAR(100) NOT NULL,
    tipo_check         NVARCHAR(10)  NOT NULL DEFAULT 'Ping',  -- Ping | Tcp | Http
    host               NVARCHAR(255) NOT NULL,  -- IP/hostname (Ping,Tcp) o URL completa (Http)
    puerto             INT           NULL,       -- solo Tcp
    ubicacion          NVARCHAR(50)  NULL,       -- texto libre: "Lima" / "Chimbote" — solo informativo
    monitorear         BIT           NOT NULL DEFAULT 1,
    activo             BIT           NOT NULL DEFAULT 1,
    creado_en          DATETIME2     NOT NULL DEFAULT GETDATE(),
